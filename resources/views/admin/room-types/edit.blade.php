...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <!-- Form -->
        <form action="{{ route('admin.room-types.update', $roomType) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="currency" 
                        ...
                        <option value="GBP" {{ old('currency', $roomType->currency) === 'GBP' ? 'selected' : '' }}>GBP (£)</option>
                        <option value="TZS" {{ old('currency', $roomType->currency) === 'TZS' ? 'selected' : '' }}>TZS (TSh)</option>
                    </select>
                    @error('currency')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="tent_type" 
                        ...
                        <option value="Small Tents" {{ old('tent_type', $roomType->tent_type) === 'Small Tents' ? 'selected' : '' }}>Small Tents</option>
                    </select>
                    @error('tent_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        @foreach(['image_1_url', 'image_2_url', 'image_3_url'] as $index => $imageAttr)
                            @if($roomType->$imageAttr)
                                <div class="relative">
                                    <img src="{{ $roomType->$imageAttr }}" alt="Image {{ $index + 1 }}" class="w-full h-24 object-cover rounded-lg">
                                    <span class="absolute top-1 left-1 bg-gray-600 text-white text-xs px-2 py-1 rounded">{{ $index + 1 }}</span>
                                </div>
                            @endif
                        @endforeach
                    </div>
            ...
            <!-- Images Upload -->
            <div class="space-y-4">
                <h3 class="text-lg font-medium text-gray-900">Update Images</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Main Image -->
                    <div>
