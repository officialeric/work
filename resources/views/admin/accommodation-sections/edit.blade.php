...
@section('title', 'Edit Accommodation Section')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Edit Accommodation Section</h1>
            <p class="text-gray-600 mt-1">Update accommodation section details</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="p-6">
            <form action="{{ route('admin.accommodation-sections.update', $accommodationSection) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <!-- Current Images -->
                @if($accommodationSection->main_image || $accommodationSection->image_1 || $accommodationSection->image_2)
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Current Images</label>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            @if($accommodationSection->main_image)
                                <div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
                                    <img src="{{ $accommodationSection->main_image_url }}" class="w-full h-32 object-cover">
                                    <div class="p-2">
                                        <p class="text-xs text-gray-500">Main Image</p>
                                    </div>
                                </div>
                            @endif
                            @if($accommodationSection->image_1)
                                <div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
                                    <img src="{{ $accommodationSection->image_1_url }}" class="w-full h-32 object-cover">
                                    <div class="p-2">
                                        <p class="text-xs text-gray-500">Image 1</p>
                                    </div>
                                </div>
                            @endif
                            @if($accommodationSection->image_2)
                                <div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
                                    <img src="{{ $accommodationSection->image_2_url }}" class="w-full h-32 object-cover">
                                    <div class="p-2">
                                        <p class="text-xs text-gray-500">Image 2</p>
                                    </div>
                                </div>
                            @endif
                        </div>
